...
- Slices 20 and 21 are hardware validated on candidate `24a061c915977684299250ccd20b7668ab041901` and promoted with that exact product/build tree. Thor CI run `36048661976` passed; artifact ZIP SHA-256 `f87e45e01d99156b1deb072066c09006a29fa5ec3223ee92633ec92e327ffb22`; APK SHA-256 `112ffc5c94eace5c65990cb02f04dbe9ac1f067428d9247ef1a3fe12c9c4f658`. The verified APK was installed on an AYN Thor, and the user reported that the full Slice 21 checklist on the preceding candidate and the focused source-only action-15 retest on this candidate passed. The exact receipt and test history are in `AYN_THOR_BACKLOG.md`.
...
The first run on the permanent branch can be cold when no compatible entry exists. Existing evidence shows a cold candidate taking about 19m55s for CMake with `0 / 842` ccache hits (run `35581988420`), while later same-slice candidates took about 1m29s for CMake with `833 / 842` hits (`98.93%`, runs `35585305840` and `35586360399`). Slice 19 then restored the previous permanent-candidate cache and completed its ARM64 build in 216 seconds (about 3m36s) with `787 / 842` hits, 55 misses, a `93.47%` hit rate, and about `0.4 GB / 3.0 GB` cache size; it is classified `warm`. A normal small follow-up candidate should therefore be visibly warm, but the hit rate is diagnostic only: cache misses from changed compiler inputs, headers, toolchain/NDK, ABI, or configuration are correct and must not fail a build.
