...
8. Never create temporary per-slice Thor CI workflows.
9. Never create per-slice candidate branches.
10. `ci/thor-candidate-validation` is the only full ARM64 candidate transport branch.
11. Do not expose or duplicate Android signing credentials outside GitHub Actions.
12. Update relevant Thor markdown documentation to reflect the actual implementation.
13. Do not mark a slice hardware validated. Hardware validation must be performed on the physical AYN Thor.
14. Do not merge or rewrite validated history without explicit user instruction.
