...
- Current work: Slice 25 adds Hero Meeting multi-slot army redistribution. Candidate run `36233203449` exposed an Android action-mask bound regression; the local correction extends the helper through action 22. A corrected candidate run and device validation are pending.
...
- Status: `awaiting CI`. Candidate `9f2abebe94ab1e037cd927385eacf7cd191a8221` failed Thor CI run [`36233203449`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36233203449) in `ThorContextIdsTest.adventureActionIdsUseStableExplicitMasks`: Android's Java `maskFor` upper bound still stopped at action 21. The helper now includes action 22 locally. This run stopped before ARM64 packaging; no candidate APK or device result exists.
