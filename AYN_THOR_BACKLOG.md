...
## Slice 20 — Hero Meeting army exchange deck

- Status: `in progress`; the authoritative full Thor ARM64 candidate run, focused native and Android tests, and package checks have passed. Physical-device validation of the checksum-verified APK is still required, so Slice 20 is not hardware validated.
- User-visible behavior: `HERO_MEETING` shows a fitted two-column deck headed “Hero Meeting”, translated hero names, seven aligned rows per army, translated creature name and count, localized `Empty` rows, and explicit left/right arrows only for movable stacks. It uses no creature or artifact artwork and requires no scrolling.
- Boundary and typed contract: native `CExchangeWindow` owns a revision-bound `ThorHeroMeetingArmy` containing exactly two stable hero IDs/names and fourteen fixed source-slot records. Keys 0–6 identify left slots and 7–13 right slots. Each bounded record carries side, slot, occupancy, creature identity/name, count, and normal-transfer availability; malformed Android payloads fail closed as a whole.
- Stable action: IDs 0–14 remain unchanged. `HERO_MEETING_MOVE_STACK = 15` uses mask `16384` and the existing target integer as the published source-slot key.
- Native responsibility and validation: activation and garrison refresh publish semantic snapshots without identical-state revision churn. MainGUI verifies exact revision/context/top `CExchangeWindow`, enabled action, key, both hero identities, decoded bounds, current stack/creature, destination ownership and a valid ordinary destination. Acceptance immediately consumes the revision and queue, then calls the unmodified `CExchangeController::moveStack` route; keyboard modifiers, synthetic events, and Android-side game rules are excluded.
- Android responsibility: JNI publishes a dedicated defensively copied immutable model. The controller pairs it only with the matching current Hero Meeting revision and clears it across revisions/contexts/lifecycle. The presentation draws only fixed text rows and creates hit behavior only for occupied, enabled transfer rows.
- Automated acceptance: the full Thor ARM64 candidate gate passed, including focused native and Android tests and package verification. Coverage includes ID/mask parity (including action 15 and mask 16384), exact-context request validation, stale/missing targets, key encoding, side identity, and bounds. Android source/model checks cover exact fourteen-slot validation, defensive copies, null/length/side/slot/key/count/duplicate rejection, stale-context clearing, fixed seven-row geometry, and unchanged earlier IDs. The native `armySlots` name avoids Qt's `slots` macro without changing the Java/JNI transport, and Thor-only lifecycle snapshots remain excluded from non-Thor builds.
- Regression risks: ownership or armies changing between render and tap, duplicate taps, long UTF-8 names, parent restoration, accidentally honoring physical modifiers, stale hit regions, and interference with Adventure/Battle decks or the upper exchange/artifact controls.
- Hardware checklist: compare both seven-slot armies and translated names/counts; move stacks in both directions into empty/merge/swap cases; verify last-stack behavior matches the upper normal arrow; verify unavailable/empty rows are inert; change ownership/availability where practical; rapid double-tap and context-close tests; display toggle/background/recreation; long localization fitting; and complete upper quick-exchange modifier, artifact, touchscreen, keyboard/controller plus Adventure/Hero/Town/Battle regressions. Do not mark validated until the checksum-verified CI APK passes these checks on a physical AYN Thor.
