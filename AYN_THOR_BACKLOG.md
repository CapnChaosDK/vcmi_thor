...
- Phase: Slices 1 through 25 are promoted. Slice 25 is CI- and hardware-validated on candidate `fb8e362e09c7aadeeeae19514f71050919920f69`; PR #11 merged into the product branch as `b5b9f664e` on 2026-09-26. This checkout retains the exact validated product/build tree plus historical receipt commits.
- Status: `hardware validated` through Slice 25 on the exact candidate. Slice 26 implementation is awaiting CI and device validation.
...
- Status: `awaiting CI`. Initial candidate `1bc0883651b0c2946948d81f4feb408ca5969ad0` passed preflight and ARM64 packaging in Thor run [`36239720953`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36239720953), then failed its focused Android test step because the newly registered pre-existing `ThorHeroMeetingRedistributionStateTest` incorrectly treated an occupied Archer stack as an invalid source. The corrected test uses an empty source; all 26 isolated Hero Meeting JVM tests pass locally. A corrected candidate and physical AYN Thor validation remain pending. Slice 25's candidate and hardware receipts above remain historical.
