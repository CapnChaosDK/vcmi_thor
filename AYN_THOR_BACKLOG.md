...
- Current work: Slice 28 adds player-installed hero portraits to the Adventure, Hero Window, and Hero Meeting decks. Implementation PR #15 is open. Candidate `74294f8af87256d1170f1fdcedc3dfbf0d731dfa` failed Thor CI run [`36268252948`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36268252948) during Android compilation because the refactor removed `Filesystem.h` from `CExchangeWindow.cpp`, which still uses `CResourceHandler` for its quick-exchange background check. Replacement candidate `c5bc6062b6a04b8617eeaaf5d61894af99d57c13` is running Thor CI push run [`36268779478`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36268779478); preflight is in progress and the ARM64 candidate job has not started. Device validation is pending. Slice 27's implementation, CI/device receipt, and Slice 26/25 validation history are recorded below.
...
- Slice 27 is hardware validated. Slice 28 extends its bounded player-installed visual assets to the selected Adventure hero, active Hero Window hero, and both Hero Meeting heroes. Preserve action IDs 0–25, existing gesture/action routes, and the current 64-key/64×64/32 KiB/64-entry/1 MiB limits.

## Slice 27 — Hero Meeting player-installed visual enrichment

- Status: `hardware validated` for exact candidate `950e8aaf7760641b0dedb28dd9878cb0d6c675f2`, promoted by PR #13 as product commit `9645eeac4f90c1d9e021fdc4854822478a2ad492` from baseline `7087228ecbd77516767238a6ed4076c9cb54aed6`. The live Thor capture on 2026-09-26 confirmed that the renderer resource-path candidate displayed Beholders but not Troglodytes or Harpies. Their source DEF frames all declare 32×32, but the stored pixel rectangles are 29×32 and 32×31 for Troglodytes and Harpies, versus 32×32 for Beholders. Direct PNG export used the cropped pixel rectangle while the transport declared the full frame, so Android rejected the mismatched PNGs. The follow-up draws onto a transparent canvas at the declared frame size before PNG export.
- Exact CI candidate: `950e8aaf7760641b0dedb28dd9878cb0d6c675f2` passed Thor CI run [`36261377889`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36261377889), including focused native and Android tests, ARM64 packaging, and package ID `is.xyz.vcmi.thor`. Artifact [`thor-candidate-arm64-36261377889`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36261377889/artifacts/10912542850) has ZIP SHA-256 `c917a087dbd24fa1cd240b1e6b92f46450e8b127918f44eee8d56be10286532c`; APK SHA-256 `f2898315c39c648619f9a288f6d53872bc5914fb0809b6bc52a868948caf03be`. The APK was checksum-verified against the CI receipt, installed on the AYN Thor, and reverified from its installed `base.apk`; package launch passed. The user confirmed Troglodyte, Harpy, and Beholder icons, and a live second-screen capture showed all three beside their occupied rows without cyan backgrounds. On 2026-09-26 the user confirmed the remaining device checklist for this exact APK: rapid/stale gestures; split, redistribution, move-all, and swap; upper-screen item changes; modal open/close; controller; and single-display behavior. Together with the previously confirmed Artifact pages, common army/artifact transfers, lower-panel toggle, background/resume, and upper controls, the complete checklist passed.
- Previous renderer-path candidate: `db84d75f36f1b1940ab2d442fbec6fcbf27df6aa` passed Thor CI run [`36259554424`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36259554424), including focused native and Android tests, ARM64 packaging, and package ID `is.xyz.vcmi.thor`. Artifact [`thor-candidate-arm64-36259554424`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36259554424/artifacts/10911529119) has ZIP SHA-256 `71378e1ab8f1c096536359bf48725ea47d78d581b8e312a583ca21bd300ec9d0`; APK SHA-256 `1ee86ff1c39ecc0545c2396428c3309cc9fadaa577c5a4b6b8c4dd5f66c8a703`. The APK was checksum-verified against the CI receipt, installed on the AYN Thor, and reverified from its installed `base.apk`; package launch passed, but the incomplete icons failed visual validation.
- Previous cache-path candidate: `ee08cb491b3982ee744b6a815ea05f5570fe71e0` passed Thor CI run [`36258200584`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36258200584), including focused native and Android tests, ARM64 packaging, and package ID `is.xyz.vcmi.thor`. Artifact [`thor-candidate-arm64-36258200584`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36258200584/artifacts/10911452730) has ZIP SHA-256 `ce4b61c1797eb12b54fc938167afdbb265dfe97cb56caf9b345415542b5ba759`; APK SHA-256 `9f60e0522cc2f3c0f19a5359db82db8bf4d3d679ee1be53d701ce0e08def24f0`. The checksum-verified APK was installed on the AYN Thor, but its text-only Hero Meeting failed visual validation.
- Previous candidate: `abc841446cebf3ae5946e6410019f98761a538bd` passed full Thor CI run [`36253013770`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36253013770). Artifact [`thor-candidate-arm64-36253013770`](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36253013770/artifacts/10909512825) has ZIP SHA-256 `f7f1163b36553e5ab551b4c4ed9e365580d37d375c825829759789924ea7762f`; its APK SHA-256 is `a42d48e7aaa8e2c214689d21e47d86a6c705077f06aff7352383b73620d422c6`. The second-screen Hero Meeting capture showed occupied Army rows with text fallback and no icons. The native follow-up moves PNG staging from the process-global temporary directory to VCMI's app-private cache. Slice 26 / PR #12 is merged into `ayn-thor-dual-screen`.
...
- Status: `awaiting CI` on product baseline `9645eeac4f90c1d9e021fdc4854822478a2ad492`. Implementation PR [#15](https://github.com/CapnChaosDK/vcmi_thor/pull/15) is open. First candidate `dcf130cbcf05884d9fd551e7e97c8765c93fe13e` failed run [36266461474](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36266461474) because the shared publisher used the wrong SDL3 renderer include root and omitted `GameEngine.h`. Candidate `64ad69405653fc03e58362996d9a302610d1c4fe` failed run [36266999948](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36266999948) because `IRenderHandler` remained incomplete. Candidate `c57b687883823cba2e6739b645fab59f0b9ff05f` failed run [36267440750](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36267440750) because `CAnimation` remained incomplete at `size()` and `getImage()` calls. Candidate `74294f8af87256d1170f1fdcedc3dfbf0d731dfa` failed run [36268252948](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36268252948) because `CExchangeWindow.cpp` still uses `CResourceHandler` after Slice 28 removed its `Filesystem.h` include during publisher extraction. That direct include is restored. Exact candidate `c5bc6062b6a04b8617eeaaf5d61894af99d57c13` is in Thor CI run [36268779478](https://github.com/CapnChaosDK/vcmi_thor/actions/runs/36268779478); its preflight is in progress, before the ARM64 candidate job. A green PR preflight does not validate or package the APK. Hardware validation remains pending.
...
- Bounded shared transport: generalize the Slice 27 visual publisher/cache for Adventure, Hero Window, and Hero Meeting. A Hero Meeting snapshot can reference up to 64 unique assets: 14 creature rows, 48 artifact rows, and two portraits. Each PNG remains at most 64×64 pixels and 32 KiB; native and Android LRU caches remain at most 64 entries and 1 MiB. Preserve transparent full-frame PNG export, app-private staging, Android decode checks, negative caching, and text fallback.
- Native responsibility: derive portrait keys from each exact displayed hero's portrait source; include the key in the semantic context or ordered Hero Meeting snapshot; collect only current-context keys; use existing `PortraitsSmall` frame indices; publish through the shared bounded pipeline. No new action IDs, action-mask bits, packets, game mutation, or asset files.
- Android responsibility: validate `HERO` key parity and PNG bounds; decode into the existing LRU; check current revision/context references before caching a delivery; draw fitted decorative portraits beside the selected Adventure header, Hero Window name, and each Hero Meeting side name. Preserve existing row geometry, Hero Meeting Army/Artifacts mode, touch targets, upper-screen paths, and text fallback.
- Automated acceptance: native stable HERO key/resource mapping; portrait-source identity and left/right association; 64-key bounds; payload validation, unavailable fallback, cache reuse/eviction, and context revision changes. Android key parity, exact two-side association, malformed/unavailable handling, cache behavior, and stale revision/context reference rejection. Retain the Slice 27 creature/artifact visual suite plus Adventure, Hero Window, Hero Meeting context, and interaction contract tests.
- Regression risks: using hero-instance identity instead of portrait-source type identity; an Adventure selection refresh pairing new text with old art; swapping Hero Meeting left/right portraits; cache key/path mismatch; stale asynchronous delivery; narrowed or overlapped hero names; altered artifact/army touch geometry; upper input/focus regression; and single-display fallback.

### Required AYN Thor hardware checklist

1. In Adventure, verify the selected hero's portrait matches the upper display; change selection rapidly and confirm the portrait and existing name/movement summary update together.
2. Open Hero Window for several heroes, including a map-specific portrait override if available; verify the portrait belongs to the exact active window hero and existing details remain visible.
3. Open Hero Meeting and verify the correct left and right portraits in Army and Artifacts modes. Change one or both heroes and confirm no side swaps or stale portraits.
4. Exercise Hero Meeting Army and Artifact interactions, including transfer, drag, split, redistribution, move-all, and swap. Confirm portrait drawing does not change mode state, row layout, gestures, or action hit regions.
5. Rapidly change hero selection, switch hero windows, open/close modals, and return to Adventure/Hero Meeting. Confirm old deliveries never render under a newer context/revision and modal restoration shows the correct current portrait.
6. Toggle the lower panel off/on and reconnect the presentation display; verify one current portrait set is shown.
7. Background/resume with each of the three surfaces active; confirm current portraits restore without stale images or focus changes.
