...
- Status: `in progress`; implemented in Cloud and awaiting authoritative Thor CI plus physical-device validation. It is not hardware validated.
...
- Automated acceptance: focused native coverage includes ID/mask parity, exact-context request validation, stale/missing targets, key encoding, side identity, and bounds. Android source/model checks cover exact fourteen-slot validation, defensive copies, null/length/side/slot/key/count/duplicate rejection, stale-context clearing, fixed seven-row geometry, and unchanged earlier IDs.
